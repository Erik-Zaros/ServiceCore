...
class Agendamento
{
    ...
    public function salvar()
    {
        ...
        $sql = "INSERT INTO tbl_agendamento (data_agendamento, hora_inicio, hora_fim, os, tecnico, status)
                VALUES ('{$data}', '{$ini}', '{$fim}', {$os}, {$tec}, 'PENDENTE') RETURNING agendamento";
        $res = pg_query($con, $sql);

        return ($res && pg_num_rows($res) > 0)
            ? ['success' => true, 'id' => pg_fetch_result($res, 0, 'agendamento')]
    ...
    public static function listarTodos($posto)
    {
        ...
        $end   = $_GET['end'] ?? null;
        $where = ($start && $end) ? "WHERE a.data_agendamento BETWEEN '{$start}' AND '{$end}'" : "";
